...
import { Search, MapPin, Heart, Calendar, ArrowRight, ArrowLeft, Users } from 'lucide-react';
...
interface UpcomingExperiencesProps {
  ...
  onLoginTrigger: () => void;
  onViewMatchesForEvent: (event: TangentEvent) => void;
}
...
export default function UpcomingExperiences({
  ...
  // Browse feed: filtered by the active popular tag only.
  // (Free-text search is now an explicit action launched from the SEARCH button.)
  const feedEvents = events.filter((evt) => {
  ...
  // Search results: computed from the submitted query when SEARCH is clicked.
  const query = submittedQuery.trim().toLowerCase();
  const searchResults = events.filter((evt) => {
    if (evt.isDraft) return false;
  ...
  // Shared event card markup (reused by the browse feed and the search results page)
  const renderEventCard = (evt: TangentEvent) => {
        ...
        <div className="p-5 space-y-3">
          ...
          <div className="flex items-center justify-between pt-1">
            {isLoggedIn ? (
              hasJoined ? (
                <button
                  onClick={() => onViewMatchesForEvent(evt)}
                  className="px-5 py-2.5 rounded font-bold text-xs tracking-wide transition uppercase cursor-pointer select-none bg-emerald-600 hover:bg-emerald-700 text-white flex items-center space-x-1.5"
                  id={`feed-join-btn-${evt.id}`}
                  title="Find companions for this event"
                >
                  <span>Attending ✓</span>
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded font-bold text-xs tracking-wide transition uppercase cursor-pointer select-none bg-black hover:bg-zinc-800 text-white"
                  id={`feed-join-btn-${evt.id}`}
                >
                  Join Event
                </button>
  ...
  // ====== SEARCH RESULTS PAGE (only the results list + Back to Events) ======
  if (showResults) {
    return (
      <div className="space-y-6 max-w-xl mx-auto animate-in fade-in duration-300" id="search-results-page">

        {/* Back navigation */}
        <button
          onClick={handleBackToEvents}
          className="flex items-center space-x-2 text-zinc-600 hover:text-black text-sm font-semibold transition cursor-pointer"
          id="search-back-to-events-btn"
        >
        ...
        <div className="space-y-1.5 text-left">
          ...
          <p className="text-xs text-zinc-600 font-sans leading-relaxed">
            ...
            {submittedQuery.trim() && (
              <> for "<span className="font-semibold text-zinc-800">{submittedQuery.trim()}</span>"</>
            )}{' '}
            near Seattle, WA within {radius} miles.
          </p>
        </div>
          ...
          {searchResults.length === 0 && (
            <div className="text-center py-12 bg-white rounded-lg border border-zinc-150 p-6">
              <p className="text-xs text-zinc-650 max-w-xs mx-auto">
                No active events near Seattle, WA match your search. Try a different keyword.
              </p>
            </div>
          )}
        </div>
  ...
  // ====== DEFAULT HOME (hero search + browse feed + CTA) ======
  return (
    <div className="space-y-12 animate-in fade-in duration-300" id="upcoming-experiences-feed">
      ...
      <div className="space-y-6 my-10 max-w-xl mx-auto">
        ...
        {/* Listing cards list */}
        <div className="space-y-6">
          {feedEvents.map(renderEventCard)}

          {feedEvents.length === 0 && (
            <div className="text-center py-12 bg-white rounded-lg border border-zinc-150 p-6">
              <p className="text-xs text-zinc-650 max-w-xs mx-auto">
                No active events match this tag right now. Try another popular tag!
              </p>
            </div>
          )}
        </div>
